...
func ServiceGetBlockBodiesQuery(chain *core.BlockChain, query GetBlockBodiesRequest) []rlp.RawValue {
	...
	for lookups, hash := range query {
		...
		// Include blob sidecars in the response (EIP-4844)
...
func handleNewBlock(backend Backend, msg Decoder, peer *Peer) error {
	...
	log.Debug("Received new block", "number", ann.Block.NumberU64(), "hash", ann.Block.Hash(), "peer", peer.ID(), "sidecars", len(ann.Sidecars))
