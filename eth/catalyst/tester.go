...
// FullSyncTester is an auxiliary service that allows Gmeme to perform full sync
...
func (tester *FullSyncTester) Start() error {
	...
	go func() {
		...
		// chain head.
		err := tester.backend.Downloader().BeaconDevSync(downloader.SyncMode(ethconfig.FullSync), tester.target, tester.closed)
		if err != nil {
