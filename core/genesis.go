...
func getGenesisState(db ethdb.Database, blockhash common.Hash) (alloc types.GenesisAlloc, err error) {
	...
	case params.InsectariumGenesisHash:
		genesis = DefaultInsectariumGenesisBlock()
	}
	if genesis != nil {
...
func (g *Genesis) toBlockWithRoot(root common.Hash) *types.Block {
	...
	if conf := g.Config; conf != nil {
		...
		if conf.IsCancun(num, g.Timestamp) {
			// EIP-4844 block
			return types.NewBlock(head, &types.Body{Withdrawals: withdrawals}, nil, trie.NewStackTrie(nil)).WithSidecars(types.BlobSidecars{})
		}
	}
...
func (g *Genesis) Commit(db ethdb.Database, triedb *triedb.Database) (*types.Block, error) {
	...
	if config.PoSA != nil && len(g.ExtraData) < 32+crypto.SignatureLength {
		return nil, errors.New("can't start posa chain without signers")
	}
