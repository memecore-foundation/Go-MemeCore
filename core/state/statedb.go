...
func (s *StateDB) handleDestruction(nodes *trienode.MergedNodeSet) (map[common.Address]struct{}, error) {
	// Short circuit if gmeme is running with hash mode. This procedure can consume
...
func (s *StateDB) Commit(block uint64, deleteEmptyObjects bool) (common.Hash, error) {
	...
	if s.snap != nil {
		start := time.Now()
		// Only update if there's a state transition (skip empty Clique and PoSA blocks)
