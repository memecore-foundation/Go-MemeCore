...
// It can be used for testing that restart Gmeme normally.
...
// It can be used for testing that restart Gmeme after the crash.
...
func (snaptest *crashSnapshotTest) test(t *testing.T) {
	...
	newchain, err := NewBlockChain(newdb, config, snaptest.gspec, nil, snaptest.engine, vm.Config{}, nil, nil)
	if err != nil {
		t.Fatalf("Failed to recreate chain: %v", err)
	}
	newchain.Stop()
...
// Tests a Gmeme restart with valid snapshot. Before the shutdown, all snapshot
...
// Tests a Gmeme was crashed and restarts with a broken snapshot. In this case the
...
// Tests a Gmeme was crashed and restarts with a broken snapshot. In this case
...
// Tests a Gmeme was running with snapshot enabled. Then restarts without
...
// Tests the Gmeme was running with snapshot enabled and resetHead is applied.
...
// Tests the Gmeme was running with a complete snapshot and then imports a few
