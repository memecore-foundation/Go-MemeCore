...
func TestEthSuite(t *testing.T) {
	...
	gmeme, err := runGmeme("./testdata", jwtPath)
	if err != nil {
		t.Fatalf("could not run gmeme: %v", err)
	}
	defer gmeme.Close()

	suite, err := NewSuite(gmeme.Server().Self(), "./testdata", gmeme.HTTPAuthEndpoint(), common.Bytes2Hex(secret[:]))
	if err != nil {
...
// runGmeme creates and starts a gmeme node
func runGmeme(dir string, jwtPath string) (*node.Node, error) {
...
func setupGmeme(stack *node.Node, dir string) error {
