...
var (
	...
	consoleCommand = &cli.Command{
		...
		Description: `
The Gmeme console is an interactive shell for the JavaScript runtime environment
which exposes a node admin interface as well as the Ðapp JavaScript API.
See https://geth.ethereum.org/docs/interacting-with-gmeme/javascript-console.`,
	}
...
which exposes a node admin interface as well as the Ðapp JavaScript API.
See https://geth.ethereum.org/docs/interacting-with-gmeme/javascript-console.
This command allows to open a console on a running gmeme node.`,
	}
...
JavaScript API. See https://geth.ethereum.org/docs/interacting-with-gmeme/javascript-console`,
	}
)

// localConsole starts a new gmeme node, attaching a JavaScript console to it at the
// same time.
...
// remoteConsole will connect to a remote gmeme instance, attaching a JavaScript
// console to it.
func remoteConsole(ctx *cli.Context) error {
	...
	if err != nil {
		utils.Fatalf("Unable to attach to remote gmeme: %v", err)
	}
	config := console.Config{
		DataDir: utils.MakeDataDir(ctx),
		DocRoot: ctx.String(utils.JSpathFlag.Name),
...
// ephemeralConsole starts a new gmeme node, attaches an ephemeral JavaScript
