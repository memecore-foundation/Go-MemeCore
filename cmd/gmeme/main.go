...
const (
	clientIdentifier = "gmeme" // Client identifier to advertise over the network
)
...
var app = flags.NewApp("the go-memecore command line interface")

func init() {
	// Initialize the CLI app and start Gmeme
	app.Action = gmeme
	...
	app.Before = func(ctx *cli.Context) error {
		...
		flags.CheckEnvVars(ctx, app.Flags, "GMEME")
		return nil
	}
...
// gmeme is the main entry point into the system if no special subcommand is run.
// It creates a default node based on the command line arguments and runs it in
// blocking mode, waiting for it to be shut down.
func gmeme(ctx *cli.Context) error {
...
func startNode(ctx *cli.Context, stack *node.Node, backend ethapi.Backend, isConsole bool) {
	...
	// Create a client to interact with local gmeme node.
	...
	if ctx.Bool(utils.MiningEnabledFlag.Name) {
		...
		if !ok {
			utils.Fatalf("MemeCore service not running")
		}
