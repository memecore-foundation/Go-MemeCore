...
func loadConfig(file string, cfg *gmemeConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
// loadBaseConfig loads the gmemeConfig based on the given command line
// parameters and config file.
func loadBaseConfig(ctx *cli.Context) gmemeConfig {
	// Load defaults.
	cfg := gmemeConfig{
...
// makeConfigNode loads gmeme configuration and creates a blank node instance.
func makeConfigNode(ctx *cli.Context) (*node.Node, gmemeConfig) {
...
func makeFullNode(ctx *cli.Context) (*node.Node, ethapi.Backend) {
	...
	// Create gauge with gmeme system and build information
	if eth != nil { // The 'eth' backend may be nil in light mode
		var protos []string
		for _, p := range eth.Protocols() {
			protos = append(protos, fmt.Sprintf("%v/%d", p.Name, p.Version))
		}
		metrics.NewRegisteredGaugeInfo("gmeme/info", nil).Update(metrics.GaugeInfoValue{
			"arch":      runtime.GOARCH,
			"os":        runtime.GOOS,
	...
	if ctx.IsSet(utils.DeveloperFlag.Name) {
		...
		// Engine API not registered for MemeCore
		// MemeCore handles consensus internally and does not require external Beacon Clients
		log.Info("Engine API disabled - PoSA consensus does not require external Beacon Clients")
	}
...
func applyMetricConfig(ctx *cli.Context, cfg *gmemeConfig) {
