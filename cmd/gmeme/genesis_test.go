...
// Tests that initializing Gmeme with a custom genesis block and chain definitions
// work properly.
func TestCustomGenesis(t *testing.T) {
	...
	for i, tt := range customGenesisTests {
		...
		runGmeme(t, "--datadir", datadir, "init", json).WaitExit()
...
func TestCustomBackend(t *testing.T) {
	...
	testfunc := func(t *testing.T, tt backendTest) error {
		...
		{ // Init
			...
			gmeme.ExpectRegexp(tt.initExpect)
			gmeme.ExpectExit()
		}
		{ // Exec + query
			...
			gmeme := runGmeme(t, args...)
			gmeme.ExpectRegexp(tt.execExpect)
			gmeme.ExpectExit()
		}
		return nil
	}
	for i, tt := range []backendTest{
		...
		{ // Can't start pebble on top of leveldb
			initArgs:   []string{"--db.engine", "leveldb"},
			execArgs:   []string{"--db.engine", "pebble"},
			execExpect: `Fatal: Failed to register the MemeCore service: db.engine choice was pebble but found pre-existing leveldb database in specified data directory`,
		},
