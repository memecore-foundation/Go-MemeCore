...
GLOBAL OPTIONS:
   ...
   --chainid value         Chain id to use for signing (4352=mainnet) (default: 4352)
...
The general flow for signing a transaction using e.g. Gmeme is as follows:
![image](sign_flow.png)

In this case, `gmeme` would be started with `--signer http://localhost:8550` and would relay requests to `eth.sendTransaction`.
...
* Another potential thing to introduce is pairing.
  ...
  * Thus Gmeme/cpp would cryptographically handshake and afterwards the caller would be allowed to make signing requests.
...
Clef listens to HTTP requests on `http.addr`:`http.port` (or to IPC on `ipcpath`), with the same JSON-RPC standard as Gmeme. The messages are expected to be [JSON-RPC 2.0 standard](https://www.jsonrpc.org/specification).
