...
*Note, to make this example work, please use you own accounts. You can create a new account either via Clef or the traditional account CLI tools. If the latter was chosen, make sure both Clef and Gmeme use the same keystore by specifying `--keystore path/to/your/keystore` when running Clef.*
...
Until then however, we're trying to pave the way via Gmeme. Gmeme v1.9.0 has built in support via `--signer <API endpoint>` for using a local or remote Clef instance as an account backend!

We can try this by running Clef with our previous rules on Rinkeby (for now it's a good idea to allow auto-listing accounts, since Gmeme likes to retrieve them once in a while).

```text
$ clef --keystore ~/.ethereum/rinkeby/keystore --chainid 4 --rules rules.js
```

In a different window we can start Gmeme, list our accounts, even list our wallets to see where the accounts originate from:

```text
$ gmeme --rinkeby --signer=~/.clef/clef.ipc console
...
*Note, if you enable the external signer backend in Gmeme, all other account management is disabled. This is because long term we want to remove account management from Gmeme.*
