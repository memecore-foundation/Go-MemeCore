...
func newTestServer() *Server {
	...
	if err := server.RegisterNameWithFilter("test", new(testService), nil); err != nil {
		panic(err)
	}
	if err := server.RegisterNameWithFilter("nftest", new(notificationTestService), nil); err != nil {
		panic(err)
	}
	return server
}
