...
var (
	// Files that end up in the gmeme*.zip archive.
	gmemeArchiveFiles = []string{
		"COPYING",
		executablePath("gmeme"),
	}

	// Files that end up in the gmeme-alltools*.zip archive.
...
func doArchive(cmdline []string) {
	var (
		...
		upload  = flag.String("upload", "", `Destination to upload the archives (usually "gmemestore/builds")`)
		ext     string
	)
	flag.CommandLine.Parse(cmdline)
	switch *atype {
	...
	for _, archive := range []string{gmeme, alltools} {
...
func archiveUpload(archive string, blobstore string, signer string, signifyVar string) error {
	...
	if signifyVar != "" {
		...
		untrustedComment := "verify with gmeme-release.pub"
...
func doDocker(cmdline []string) {
	...
	//  - ethereum/client-go:latest                            - Pushes to the master branch, Gmeme only
	//  - ethereum/client-go:stable                            - Version tag publish on GitHub, Gmeme only
	//  - ethereum/client-go:alltools-latest                   - Pushes to the master branch, Gmeme & tools
	//  - ethereum/client-go:alltools-stable                   - Version tag publish on GitHub, Gmeme & tools
	//  - ethereum/client-go:release-<major>.<minor>           - Version tag publish on GitHub, Gmeme only
	//  - ethereum/client-go:alltools-release-<major>.<minor>  - Version tag publish on GitHub, Gmeme & tools
	//  - ethereum/client-go:v<major>.<minor>.<patch>          - Version tag publish on GitHub, Gmeme only
	//  - ethereum/client-go:alltools-v<major>.<minor>.<patch> - Version tag publish on GitHub, Gmeme & tools
	var tags []string
	...
	if *image {
		...
		for _, tag := range tags {
			gmemeImage := fmt.Sprintf("%s:%s-%s", *upload, tag, runtime.GOARCH)
			...
			// master branch.
			for _, img := range []string{gmemeImage, toolImage} {
			...
			build.MustRunCommand("docker", "image", "tag", fmt.Sprintf("%s:TAG", *upload), gmemeImage)
			build.MustRunCommand("docker", "image", "tag", fmt.Sprintf("%s:alltools-TAG", *upload), toolImage)
			build.MustRunCommand("docker", "push", gmemeImage)
	...
	if len(*manifest) != 0 {
		...
		for i := 0; i < 2; i++ { // 2 attempts, second is race check
			...
			for _, tag := range tags {
				for _, arch := range strings.Split(*manifest, ",") {
					gmemeImage := fmt.Sprintf("%s:%s-%s", *upload, tag, arch)
		...
		// Assemble and push the Gmeme manifest image
		for _, tag := range tags {
			gmemeImage := fmt.Sprintf("%s:%s", *upload, tag)

			var gmemeSubImages []string
			for _, arch := range strings.Split(*manifest, ",") {
				gmemeSubImages = append(gmemeSubImages, gmemeImage+"-"+arch)
			}
			build.MustRunCommand("docker", append([]string{"manifest", "create", gmemeImage}, gmemeSubImages...)...)
			build.MustRunCommand("docker", "manifest", "push", gmemeImage)
		}
...
func doDebianSource(cmdline []string) {
	var (
		...
		sshUser  = flag.String("sftp-user", "", `Username for SFTP upload (usually "gmeme-ci")`)
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = os.MkdirTemp("", "gmeme-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, file := range allToolsArchiveFiles {
		...
		if filepath.Base(file) == "gmeme.exe" {
			gmemeTool = file
		} else {
	...
	installer, err := filepath.Abs("gmeme-" + archiveBasename(*arch, params.ArchiveVersion(env.Commit)) + ".exe")
	if err != nil {
	...
	build.MustRunCommand("makensis.exe",
		...
		"/DARCH="+*arch,
		filepath.Join(*workdir, "gmeme.nsi"),
	)
...
func doPurge(cmdline []string) {
	var (
		store = flag.String("store", "", `Destination from where to purge archives (usually "gmemestore/builds")`)
