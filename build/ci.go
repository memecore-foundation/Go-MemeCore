...
func doArchive(cmdline []string) {
	...
	var (
		env       = build.Env()
		basegmeme = archiveBasename(*arch, version.Archive(env.Commit))
...
func doDockerBuildx(cmdline []string) {
	...
	for _, spec := range []struct {
		...
		for _, tag := range tags { // latest, stable etc
			gmemeImage := fmt.Sprintf("%s%s", spec.base, tag)
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	installer, err := filepath.Abs("gmeme-" + archiveBasename(*arch, version.Archive(env.Commit)) + ".exe")
	if err != nil {
