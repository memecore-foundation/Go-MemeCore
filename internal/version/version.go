...
func versionInfo(info *debug.BuildInfo) string {
	// If the main package is from our repo, prefix version with "gmeme".
	if strings.HasPrefix(info.Path, ourPath) {
		return fmt.Sprintf("gmeme %s", info.Main.Version)
	}
	...
	if mod == nil {
		...
		// version.
		return version + fmt.Sprintf("gmeme %s", params.VersionWithMeta)
	}
